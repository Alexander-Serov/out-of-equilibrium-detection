...
def plot_link_strength_dependence(trials=20, n12_range=[1e-1, 1e3], verbose=False, recalculate_trajectory=False, recalculate_BF=False, dry_run=False, cluster=False, rotation=True):
    """
            ...
            for ind_M, M in enumerate(Ms):
                for ind_n12, n12 in enumerate(n12s):
                    args_string = get_cluster_args_string(
                        D1=D1, D2=D2, n1=n1, n2=n2, n12=n12, gamma=gamma,  dt=dt, angle=angle, L=L, trial=trial, M=M, verbose=verbose, recalculate_trajectory=recalculate_trajectory, recalculate_BF=recalculate_BF, rotation=rotation)
                    # print('Calculating with parameters: ', args_string)
                    lg_BF_vals[ind_M, ind_n12, trial], ln_evidence_with_link, ln_evidence_free, loaded, dict_data = simulate_and_calculate_Bayes_factor_terminal(
...
def plot_diffusivity_dependence(trials=20, D1_range=[0.01, 10], verbose=False, recalculate_trajectory=False, recalculate_BF=False, dry_run=False, cluster=False, rotation=True):
    """
        ...
        for trial in trange(trials, desc='Loading/scheduling calculations'):
            ...
            for ind_M, M in enumerate(Ms):
                for ind_D1, D1 in enumerate(D1s):
                    args_string = get_cluster_args_string(
                        D1=D1, D2=D2, n1=n1, n2=n2, n12=n12, gamma=gamma, dt=dt, angle=angle, L=L, trial=trial, M=M, verbose=verbose, recalculate_trajectory=recalculate_trajectory, recalculate_BF=recalculate_BF, rotation=rotation)
                    lg_BF_vals[ind_M, ind_D1, trial], ln_evidence_with_link, ln_evidence_free, loaded, dict_data = simulate_and_calculate_Bayes_factor_terminal(
                        args_string, cluster=cluster)
                    if cluster and not loaded:
                        file.write(args_string)
...
def plot_localization_dependence(trials=20, n_range=[1e-2, 100], particle=1, verbose=False, recalculate_trajectory=False, recalculate_BF=False, dry_run=False, cluster=False, rotation=True):
    """
            ...
            for ind_M, M in enumerate(Ms):
                ...
                if particle == 1:
                    for ind_n, n1 in enumerate(n1s):
                        ...
                        lg_BF_vals[ind_M, ind_n, trial], ln_evidence_with_link, ln_evidence_free, loaded, dict_data = simulate_and_calculate_Bayes_factor_terminal(
                            args_string, cluster=cluster)
                        if cluster and not loaded:
                            file.write(args_string)
...
def plot_angle_dependence(trials=20, verbose=False, recalculate_trajectory=False, recalculate_BF=False, dry_run=False, cluster=False):
    """
    The function loads data for the specified parameters and plots the diffusivity dependence plot.
        ...
        for trial in trange(trials, desc='Loading/scheduling calculations'):
            ...
            for ind_M, M in enumerate(Ms):
                for ind_alpha, alpha in enumerate(alphas):
                    args_string = get_cluster_args_string(
                        D1=D1, D2=D2, n1=n1, n2=n2, n12=n12, gamma=gamma, dt=dt, angle=alpha, L=L, trial=trial, M=M, verbose=verbose, recalculate_trajectory=recalculate_trajectory, recalculate_BF=recalculate_BF, rotation=rotation)
                    lg_BF_vals[ind_M, ind_alpha, trial], ln_evidence_with_link, ln_evidence_free, loaded, dict_data = simulate_and_calculate_Bayes_factor_terminal(
                        args_string, cluster=cluster)
                    ...
                    if 'MLE_link' in dict_data:
                        ...
                        MLE_alphas_centered[ind_M, ind_alpha,
                                            trial] = (dict_data['MLE_link']['alpha'] - alpha) / np.pi * 180
